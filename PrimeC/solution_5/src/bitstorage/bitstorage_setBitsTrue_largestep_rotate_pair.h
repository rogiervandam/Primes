...
static void
NAME(create_mask_largestep_rotate_pair,suffix)(void* restrict bitstorage, const counter_t range_start, const counter_t step, const counter_t range_stop)
{
    ...
    for (counter_t index = range_start; index <= range_stop_unique_vector; current_vector++) {
        ...
        if (current_vector &1 || index > range_stop_unique_vector) {
            NAME(applyMask,suffix)(bitstorage_vector, step, range_stop, mask_vector, current_vector);
        }
        else {
            counter_t next_vector_start = vectorstart_type(index,bitbucket_t);
            bitbucket_t mask_nextvector = BITBUCKET_BASE((variant_base_type_t) 0U);
            for (counter_t i=0; i < BITBUCKET_ELEMENTS; i++) {
                if (vectorstart_type(index,variant_base_type_t) == (next_vector_start + (bitcount_type(variant_base_type_t)*i))) {
                    mask_nextvector[i] = markmask_calc_type(index, variant_base_type_t); // in clang, markmask_type is enough, not in gcc
                    index += step;
                }
            }
            NAME(applyMask_pair,suffix)(bitstorage_vector, step, range_stop, mask_vector, mask_nextvector, current_vector);
            current_vector++;
        }
    }
...
static void
NAME(setBitsTrue_largestep_rotate_pair,suffix)(void* restrict bitstorage, const counter_t range_start, const counter_t step, const counter_t range_stop) 
{
    ...
    NAME(create_mask_largestep_rotate_pair,suffix)(bitstorage, range_start_new, step, range_stop);
