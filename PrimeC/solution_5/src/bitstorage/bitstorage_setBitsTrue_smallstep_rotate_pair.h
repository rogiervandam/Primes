...
{
    logBegin7(time_setBitsTrue_smallstep_rotate_pair, "SetBitsTrueSmallstepRotatePair: setting bits step %3ju using smallstep%-10s in %ju bit range (%ju-%ju) with %ju bits to set; using %ju copies of %ju bit mask", (uintmax_t)step, STR(suffix), (uintmax_t)safe_diff(range_stop,range_start),(uintmax_t)range_start,(uintmax_t)range_stop, (uintmax_t)((safe_diff(range_stop,range_start))/(uintmax_t)step), (uintmax_t)(((uintmax_t)safe_diff(range_stop,range_start))/(uintmax_t)(bitcount_type(bitbucket_t)*step)), (uintmax_t)bitcount_type(bitbucket_t));
    ...
    if (range_start_nexttvector + step * bitcount_type(bitbucket_t) > range_stop) {
        ...
        logEnd7(time_setBitsTrue_smallstep_rotate_pair, "\n");
        return;
    }
