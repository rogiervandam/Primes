...
continuePattern_shiftleft_unrolled(bitword_t* restrict bitstorage, const counter_t aligned_copy_word, const bitshift_t shift, counter_t copy_word, counter_t source_word) 
{
    ...
    while (copy_word < fast_loop_stop_word) {
        register const bitword_t source0 = bitstorage_sized[source_word  ];
        register const bitword_t source1 = bitstorage_sized[source_word+1];
        bitstorage_sized[copy_word  ] = (source0 >> shift) | (source1 << shift_flipped);
        register const bitword_t source2 = bitstorage_sized[source_word+2];
