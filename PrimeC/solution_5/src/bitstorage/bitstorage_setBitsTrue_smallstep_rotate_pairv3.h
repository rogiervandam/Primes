...
static inline void __attribute__((always_inline)) NAME(applyMask,suffix)(void* restrict bitstorage, const counter_t step, const counter_t range_stop, const bitbucket_t mask1, const bitbucket_t mask2, const bitbucket_t mask3, const bitbucket_t mask4, counter_t index_vector) 
{
    ...
    while likely(index_ptr < fast_loop_ptr) {
        ...
        *(index_ptr + step + 3  ) |= mask4;  
        *(index_ptr + step_2)     |= mask1; 
        *(index_ptr + step_2 + 1) |= mask2;  
        *(index_ptr + step_2 + 2) |= mask3;  
        *(index_ptr + step_2 + 3) |= mask4;  
        *(index_ptr + step_3)     |= mask1; 
        *(index_ptr + step_3 + 1) |= mask2;  
        *(index_ptr + step_3 + 2) |= mask3;  
        *(index_ptr + step_3 + 3) |= mask4;  
        #if unrolls <= 4
        index_ptr += step_max;
        #else
...
static inline void __attribute__((always_inline)) NAME(create_mask_smallstep,suffix)(void* restrict bitstorage, const counter_t range_start, const counter_t step, const counter_t range_stop)
{
    ...
    for (; current_vector < vector_max; current_vector += 4) {
        bitbucket_t mask_vector2 = (mask_vector << pattern_vectorshift_vector) | (mask_vector >> (step_shift_vector - pattern_vectorshift_vector)); 
        bitbucket_t mask_vector3 = (mask_vector2 << pattern_vectorshift_vector) | (mask_vector2 >> (step_shift_vector - pattern_vectorshift_vector));
        bitbucket_t mask_vector4 = (mask_vector3 << pattern_vectorshift_vector) | (mask_vector3 >> (step_shift_vector - pattern_vectorshift_vector));        
        NAME(applyMask,suffix)(bitstorage_vector, step, range_stop, mask_vector, mask_vector2, mask_vector3, mask_vector4, current_vector);
        mask_vector = (mask_vector4 << pattern_vectorshift_vector) | (mask_vector4 >> (step_shift_vector - pattern_vectorshift_vector)); 
    }
