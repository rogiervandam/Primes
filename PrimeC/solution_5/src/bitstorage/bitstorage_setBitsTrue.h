...
setBitsTrue_v256(void* restrict bitstorage, const counter_t range_start, const counter_t step, const counter_t range_stop) 
{
    ...
    if (step < 64) {
        ...
        if (step  < 128 && step < global_largestep_faster)  { setBitsTrue_largestep_vector_uint64v4_unroll4(bitstorage, range_start, step, range_stop); return; }
        else if (step  < 512 && step < global_largestep_faster)  { setBitsTrue_largestep_vector_uint64v8_unroll4(bitstorage, range_start, step, range_stop); return; }
    }

    const counter_t range = range_stop - range_start, ratio = range / step;
    if      (ratio > 128) { setBitsTrue_largestep_repeat_uint8_unroll16 (bitstorage, range_start, step, range_stop); } 
    else if (ratio >  32) { setBitsTrue_largestep_repeat_uint8         (bitstorage, range_start, step, range_stop); } 
    else                    setBitsTrue_largestep_norepeat_uint8       (bitstorage, range_start, step, range_stop);
...
setBitsTrue_v512(void* restrict bitstorage, const counter_t range_start, const counter_t step, const counter_t range_stop) 
{
    ...
    if      (step  <  16)  setBitsTrue_smallstep_rotate_pair_uint16v32_unroll8(bitstorage, range_start, step, range_stop);
    else if (step  <  32)  setBitsTrue_smallstep_rotate_pair_uint32v16_unroll8(bitstorage, range_start, step, range_stop);
    else if (step  <  64)  setBitsTrue_smallstep_rotate_pair_uint64v8_unroll8 (bitstorage, range_start, step, range_stop); 
    else if (step  < 512 && step < global_largestep_faster)  setBitsTrue_largestep_vector_uint64v8_unroll4(bitstorage, range_start, step, range_stop); 
    else {
        // setBitsTrue_largestep_repeat_uint8         (bitstorage, range_start, step, range_stop);
        // return;
...
{
    setBitsTrue_v256 (bitstorage, range_start, step, range_stop);
