...
function(continuePattern_smallSize,suffix)(void* restrict bitstorage, const counter_t source_start, const counter_t destination_stop, const counter_t size)
{
    log7(time_continuePattern_smallSize, bitstorage, "ContinuePatternSmallSize: continue pattern size %ju in %ju bit range (%ju-%ju) using continuePattern_smallSize (%ju copies)", (uintmax_t)size, (uintmax_t)destination_stop-(uintmax_t)source_start,(uintmax_t)source_start,(uintmax_t)destination_stop, (uintmax_t)(((uintmax_t)destination_stop-(uintmax_t)source_start)/(uintmax_t)size));
    ...
    if (destination_start_word >= destination_stop_word) {
        ...
        log9(time_continuePattern_smallSize, bitstorage, "ContinuePatternSmallSize: handled with small size in one word destination_start=%ju destination_stop=%ju size=%ju", (uintmax_t)destination_start, (uintmax_t)destination_stop, (uintmax_t)size);
        log7(time_continuePattern_smallSize, bitstorage);
        return;
    }

    bitstorage_sized[destination_start_word++] |= (pattern << bitindex_type(destination_start, bitbucket_t));
    log9(time_continuePattern_smallSize, bitstorage,
    ...
    for (counter_t i = 0; i <= loop_range; ++i ) {
        ...
        log9(time_continuePattern_smallSize, bitstorage, "ContinuePatternSmallSize: handled with small size in loop destination_start=%ju destination_stop=%ju size=%ju loop_index=%ju", (uintmax_t)destination_start, (uintmax_t)destination_stop, (uintmax_t)size, (uintmax_t)i);  
    }
