...
{
    logBegin8(time_applyMask_pair, "ApplyMaskPair: apply %s (%ju bit) mask in pairs with step %ju in bitrange (%ju - %ju)", STR(bitbucket_t), bitcount_type(bitbucket_t), (uintmax_t)step, (uintmax_t)range_start * bitcount_type(bitbucket_t), (uintmax_t)(range_stop + 1) * bitcount_type(bitbucket_t) - 1);
