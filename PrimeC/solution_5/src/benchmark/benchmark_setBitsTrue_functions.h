        ...
        for(int method=0; method<=methods; method++) {
            ...
            while (time_elapsed <= time_target) {
                switch(method) {
                    ...
                    case 14: setBitsTrue_largestep_repeat_uint32_unroll4(bitstorage, start, step, block_stop); passes++; break;
                    case 15: setBitsTrue_largestep_repeat_uint64_unroll4(bitstorage, start, step, block_stop); passes++; break;
                    case 16: setBitsTrue_largestep_repeat_uint8_unroll8(bitstorage, start, step, block_stop); passes++; break;
    ...
    for(int step=1; step<prime_max*2+1; step+=2) {
        ...
        if (checkBitFalse(bitstorage, prime)==0 ) {
            ...
            for(int method=0; method<=methods; method++) {
                ...
                } else if (method >= nonvector && stripe_passes[step][method] >= max_value * 0.97 && stripe_passes[step][method] < max_value && max_value > 0) {
                    printf("\033[33m%6ju\033[0m ", (uintmax_t)stripe_passes[step][method]); // Yellow for within 5% of max
                } else if (method == 0 && max_value > 0 && stripe_passes[step][0] <= max_value * 0.97) {
