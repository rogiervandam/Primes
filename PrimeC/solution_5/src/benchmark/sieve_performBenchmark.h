...
emitEmbedHeader(const char* filepath, benchmark_settings_t settings) 
{
    ...
    fprintf(f, "// Auto-generated by %s --emit-embed\n", option.program_name);
    fprintf(f, "// Compile-time constants: %s\n", settings_string);
    fprintf(f, "#ifndef SIEVE_EMBED_H\n");
    fprintf(f, "#define SIEVE_EMBED_H\n\n");
    fprintf(f, "static const counter_t global_stripeprime_faster  = %ju;\n", (uintmax_t)settings.stripe_faster);
    fprintf(f, "static const counter_t global_largestep_faster    = %ju;\n", (uintmax_t)settings.largestep_faster);
    fprintf(f, "static const counter_t global_blocksize_bits      = %ju;\n", (uintmax_t)settings.blocksize_bits);
    fprintf(f, "static const counter_t global_vectorsize          = %ju;\n", (uintmax_t)settings.vectorsize);
    fprintf(f, "\n#endif // SIEVE_EMBED_H\n");
    fclose(f);
    verbose2(printf("Generated embed header: %s\n", filepath);)
}

static void __attribute__((cold))
saveLastSettings(benchmark_settings_t settings)
{
    // emit embed header if requested and exit
    if (option.emit_embed_path) {
        emitEmbedHeader(option.emit_embed_path, settings);
        // return 0;
    }
