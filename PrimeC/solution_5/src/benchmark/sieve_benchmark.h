...
{
    verbose5( printf("Starting check with benchmark settings\n"); )
    ...
    verbose5( printf("Completed check with benchmark settings\n"); )
    verbose3( if (!valid) {
    ...
    verbose5( printf("Deleting sieve\n"); )
    sieve_delete(sieve_check);
    verbose5( printf("Sieve deleted\n"); )
    return valid;
}
