// #ifndef wheelstorage_small_repeat_mmask_GUARD
//     #define wheelstorage_small_repeat_mmask_GUARD

//     #undef INCLUDE_ME
//     #define INCLUDE_ME "wheelstorage_smallrepeat_mmask.h"
...
function(markFactors_wheelstorage_small_repeat_mmask,suffix)(void* restrict bitstorage, counter_t start_number, const counter_t stop_number, const counter_t step, counter_t max_masks)
{
    ...
    for (counter_t number_index = start_number; number_index <= stop_number_unique; number_index += step) {
        ...
        // const counter_t current_bucket = index_type(((wheelmask_stripe_bits * (number_index / WHEEL_SIZE)) + abs(wheelmask_bitpoint[wheel_index] )), bitbucket_t);
        ...
        if (current_bucket > target_bucket) {
            ...
            // function(applyMask_index_mmask,suffix)(bitstorage, start_bucket, stop_bucket, wheel_step, masks, max_masks);
        ...
        // const counter_t wheel_bit = wheel_bit_calc(number_index);
        if (wheel_bit >= 0) masks[current_bucket - start_bucket] |= markmask_type(wheel_bit, bitbucket_t);
        // if (wheelmask_bitpoint[wheel_index] >= 0) {
        //     const counter_t wheel_bit = wheelmask_stripe_bits * (number_index / WHEEL_SIZE) + wheelmask_bitpoint[wheel_index];
        //     masks[current_bucket - start_bucket] |= markmask_type(wheel_bit, bitbucket_t);
        // }
    }

    const counter_t masks_remaining = min(stop_bucket - min(stop_bucket, start_bucket), max_masks);
    // if (masks_remaining) APPLYMASK_CALL(bitstorage, start_bucket, stop_bucket, wheel_step, masks);
