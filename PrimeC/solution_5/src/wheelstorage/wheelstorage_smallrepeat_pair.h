...
static inline void __attribute__((always_inline, hot, nonnull,  aligned(cache_line_bytes))) 
function(markFactors_wheelstorage_small_repeat_pairv2,suffix)(sieve_t* sieve, counter_t start_number, const counter_t stop_number, const counter_t step)
{
    logStart7(sieve->bitstorage, time_markFactors_wheelstorage_small_repeat_pairv2, "Setting factors with step %3ju for prime %ju using markFactors_wheelstorage_small_repeat_pair %s in %ju factor range (%ju-%ju) (%ju occurances; %ju repeats)", (uintmax_t)step, (uintmax_t)step/2, STR(suffix), (uintmax_t)safe_diff(stop_number,start_number),(uintmax_t)start_number,(uintmax_t)stop_number, (uintmax_t)((safe_diff(stop_number,start_number))/(uintmax_t)step), (uintmax_t)(((uintmax_t)safe_diff(stop_number,start_number))/(uintmax_t)(bitcount_type(bitbucket_t)*step)));   
    ...
    bitshift_t bitshift = bitcount_type(bitbucket_t) % step; // the bitshift to apply to the mask to get it in the correct position for the current bucket
    ...
    for (counter_t index = start_number; index <= stop_number_unique; index += step) {
        ...
        if (new_bucket != current_bucket) {
            ...
            pending_mask = current_mask;
            current_bucket = new_bucket;
            current_mask = (pending_mask >> bitshift) | (pending_mask << (bitcount_type(bitbucket_t) - bitshift)); // shift the pending mask to the correct position for the new bucket
            // log9("new_current_mask index: %ju, wheel_bit: %jd, new_bucket: %ju", (uintmax_t)index, (intmax_t)wheel_bit, (uintmax_t)new_bucket);
        }
    ...
    logStop8(sieve->bitstorage, time_markFactors_wheelstorage_small_repeat_pairv2, "finished marking factors with step %3ju for prime %ju using markFactors_wheelstorage_small_repeat_pair_vector %s in %ju factor range (%ju-%ju) (%ju occurances; %ju repeats)", 
        (uintmax_t)step, (uintmax_t)step/2, STR(suffix), (uintmax_t)safe_diff(stop_number,start_number),(uintmax_t)start_number,(uintmax_t)stop_number, (uintmax_t)((safe_diff(stop_number,start_number))/(uintmax_t)step), 
        (uintmax_t)(((uintmax_t)safe_diff(stop_number,start_number))/(uintmax_t)(bitcount_type(bitbucket_t)*step)));

    logStop7(sieve->bitstorage, time_markFactors_wheelstorage_small_repeat_pairv2, "finished setting factors");
}