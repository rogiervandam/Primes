    ...
    wheel_bit_calc(counter_t index_number) {
        const counter_t wheel_index = index_number % WHEEL_SIZE;
        if (wheelmask_bitpoint[wheel_index] < 0) return -1;
        return (wheelmask_stripe_bits * (index_number / WHEEL_SIZE)) + wheelmask_bitpoint[wheel_index];
    }

    // returns the bit index for a given number index or -(bit estimate) if the number is divisible by any of the wheel primes
    static inline counter_t __attribute__((always_inline, hot, aligned(cache_line_bytes))) 
    wheel_bit_calc_estimate(counter_t index_number) {
        const counter_t wheel_index = index_number % WHEEL_SIZE;
        if (wheelmask_bitpoint[wheel_index] < 0) return (-1 * (wheelmask_stripe_bits * (index_number / WHEEL_SIZE)) + wheelmask_bitpoint[wheel_index]); 
        return (wheelmask_stripe_bits * (index_number / WHEEL_SIZE)) + wheelmask_bitpoint[wheel_index];
    }
    ...
    static inline counter_t __attribute__((always_inline, hot, aligned(cache_line_bytes))) 
    wheel_bit_estimate_next(counter_t index_number) {
        counter_t wheel_index = index_number % WHEEL_SIZE;
        return (wheelmask_stripe_bits * (index_number / WHEEL_SIZE)) + abs(wheelmask_bitpoint[wheel_index]);
    }
    ...
    static inline counter_t __attribute__((always_inline, hot, aligned(cache_line_bytes))) 
    wheel_bit_estimate_last(counter_t index_number) {
        counter_t wheel_index = index_number % WHEEL_SIZE;
        if (wheelmask_bitpoint[wheel_index] < 0) return (wheelmask_stripe_bits * (index_number / WHEEL_SIZE)) - wheelmask_bitpoint[wheel_index] - 1;
        return (wheelmask_stripe_bits * (index_number / WHEEL_SIZE)) + wheelmask_bitpoint[wheel_index];
    }
