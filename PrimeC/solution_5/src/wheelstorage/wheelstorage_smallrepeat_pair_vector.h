...
function(markFactors_wheelstorage_small_repeat_pair_vector,suffix)(void* restrict bitstorage, counter_t range_start, const counter_t range_stop, const counter_t step)
{
    logStart6(bitstorage, time_markFactors_wheelstorage_small_repeat_pair_vector, "MarkFactorsWheelStorageSmallRepeatPairVector: setting factors step %3ju using markFactors_wheelstorage_small_repeat_pair_vector %s in %ju factor range (%ju-%ju) (%ju occurances; %ju repeats)", (uintmax_t)step, STR(suffix), (uintmax_t)safe_diff(range_stop,range_start),(uintmax_t)range_start,(uintmax_t)range_stop, (uintmax_t)((safe_diff(range_stop,range_start))/(uintmax_t)step), (uintmax_t)(((uintmax_t)safe_diff(range_stop,range_start))/(uintmax_t)(bitcount_type(bitbucket_t)*step)));
    ...
    for (; (current_bucket = function(wheel_bucket_calc,variant_suffix)(range_start)) < 1 && range_start <= range_stop_unique; range_start += step) {
        function(markFactor_wheelstorage,variant_base_suffix)(bitstorage, range_start);
    }
