    ...
    function(checkFactor_wheelstorage_unsafe,suffix)(sieve_t* sieve, register counter_t index)
    {
        ...
        if (wheelstorage_bit < 0) return 1; // if the number is divisible by any of the wheel primes, it is not prime
        return (bitstorage_sized[ index_type(wheelstorage_bit, bitbucket_t)] & markmask_type(wheelstorage_bit, bitbucket_t)) != 0;

    }

    // checks if the factor is marked in the wheelstorage, returns 1 if it is marked (not prime), 0 if it is not marked (prime)
    // safe because it returns the correct results for the very first numbers (1,2,3,5 .. WHEEL_MAX) by checking the wheel_primes array
    static inline bitbucket_t __attribute__((always_inline, hot, nonnull, aligned(cache_line_bytes)))
    function(checkFactor_wheelstorage,suffix)(sieve_t* sieve, register counter_t factor) {
        if (factor <= WHEEL_MAX) {
            return wheel_primes[factor];
        }
