...
function(markFactors_wheelstorage_repeat,suffix)(void* restrict bitstorage, const counter_t start_number, counter_t stop_number, const counter_t step)
{
    ...
    const counter_t last_number_unique = min(getFactor( (wheel_bit_estimate_last(start_number)) - 1 + wheel_step * wheelmask_stripe_bits * ((bitcount_type(bitbucket_t) + wheelmask_stripe_bits - 1) / wheelmask_stripe_bits)), stop_number);
