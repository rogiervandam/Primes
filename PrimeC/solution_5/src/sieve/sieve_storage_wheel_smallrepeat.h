    ...
    for (register counter_t index = range_start; index <= range_stop_unique; index += step) { 
        ...
        if (current_bucket < new_bucket) { // when going to the next block
            if (current_mask) { // apply previous mask if it exists
                function(applyMask_index,suffix)(sieve->bitstorage, current_bucket, block_stop, wheel_step, current_mask);
            }
        ...
        // current_mask |= markmask << ((wheel_block_calc_uint8(index) & 7) *8); // combine the markmask for the current block if it is the same as the previous one
