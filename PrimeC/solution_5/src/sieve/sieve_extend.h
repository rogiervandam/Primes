...
{
    // size the first block, optimizing for large following blocks and aligning to cache line
    counter_t block0_stop = (((sieve_size % blocksize_factor) + cache_line_bytes*8) & ~(cache_line_bytes*8-1)) * 2 + 1; 
    ...
    for (counter_t block_start = block0_stop, block_stop = block_start + blocksize_factor; block_start < sieve_size; block_start += blocksize_factor, block_stop += blocksize_factor) {
        ...
        stripeSieveBlock(sieve->bitstorage, block_start/2, min(block_stop/2, sieve_size/2), prime_start/2, min(calcFactor_max_half(block_stop/2),prime_max/2));
    }
    return prime_next; 
}