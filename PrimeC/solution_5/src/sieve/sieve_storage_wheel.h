...
#ifdef COMPILE_TRACE
        /* item 230/#225: set pending target so logStop9 can emit target_bits in the trace */
        if (g_trace.enabled && wheel_bit >= 0) primes_trace_set_pending_target((uint32_t)wheel_bit);
#endif
