...
static sieve_t* shakeSieve(const counter_t sieve_size, const storage_type storage)
{
    ...
    for (counter_t block_start = 0; block_start < sieve_size; block_start += factorBlock) {
        ...
        logStart5(sieve->bitstorage, time_wheelstorage_blockprocessing, "Processing block with range %ju - %ju\n",(uintmax_t)block_start, (uintmax_t)block_stop); 
        ...
        logStop5(sieve->bitstorage, time_wheelstorage_blockprocessing, "Finished processing block with range %ju - %ju\n",(uintmax_t)block_start, (uintmax_t)block_stop); 
    } 
