export function multiBitBounds(host, startBit, count) {
...
export function bitVisualRow(host, bitIdx) {
...
export function multiBitBoundsSegments(host, startBit, count) {
  const endBit = Math.min(host.bitCount, startBit + count);
  if (startBit < 0 || endBit <= startBit) return [];

  const segments = [];
  let segmentStart = startBit;
  let previousRow = bitVisualRow(host, startBit);

  for (let bit = startBit + 1; bit < endBit; bit++) {
    const row = bitVisualRow(host, bit);
    if (row !== previousRow) {
      const segmentCount = bit - segmentStart;
      const bounds = multiBitBounds(host, segmentStart, segmentCount);
      if (bounds) {
  ...
  const finalBounds = multiBitBounds(host, segmentStart, finalCount);
  if (finalBounds) {
...
export function getElementBounds(host, type, index) {
  ...
  if (type === 'byte') return multiBitBounds(host, index * 8, 8);
  if (type === 'uint32') return multiBitBounds(host, index * 32, 32);
  if (type === 'uint64') return multiBitBounds(host, index * 64, 64);
