 ...
 *
 * - `playbackSpeed` slider maps log-scaled to a percentage 1..1600 (where
...
/** Map slider value (1..100) → playback speed percentage (1..1600). */
export function playbackSpeedToPercent(speedValue) {
  ...
  return Math.round(Math.pow(1600, ratio));
}

/** Inverse: percentage → slider value. */
export function percentToPlaybackSpeed(pctValue) {
  const pct = clamp(parseInt(pctValue || 0, 10) || 100, 1, 1600);
  const ratio = Math.log(pct) / Math.log(1600);
