...
241 Make it clearer that repeat mode is active. For example, when repeat mode is on, the play button could have a different color or a small badge that says "Repeat" on it. This way users can easily see that repeat mode is enabled and understand that the current event will keep repeating until they disable it or move to the next event manually. This can help prevent confusion and make it easier for users to control the animation playback according to their preferences.
Fixed: Added `dtl-repeat-on` CSS class to play button when `isSingleEventRepeatEnabled` is true. Play button gets a colored ring (`box-shadow`). Repeat button gets a pulsing glow animation (`dtl-repeat-active-pulse` @keyframes) and a small colored badge dot (::before pseudo-element) when active.

242 Both timelines should have the same blurry effect as the title and annotation, so the user can get a sense of depth and focus when looking at the timelines. 
Fixed: Increased `backdrop-filter` blur from 6px → 12px and reduced background opacity from 0.88 → 0.65 for both `.dtl-wave-zone` and `.dtl-anim-zone`. Also replaced the opaque canvas fill (`ctx.fillStyle = '#0e1b2e'; ctx.fillRect(...)`) with `ctx.clearRect(...)` so the wave zone canvas is transparent and the CSS glassmorphism shows through.

243 The gameplay modes in #237 collides with other key assignments, like D for detail panel. Therefore make a Fly mode, activated and deactivated by using F key. In fly. mode, key assigngments are different. When entering fly mode, note which panels are open so they can be restored when exiting fly mode, and close any open panels to give the user a clear view for navigation.  In fly mode, the mouse controls the camera direction and the WASD keys control the movement, while in normal mode, the keys work as they do now. This way users can easily switch between exploring the trace with fly mode and using the regular controls for interacting with the panels and timelines without conflicts. Put in other controls to move up and down, zoom in and out. When exiting fly mode, restore the panels to their previous open/closed state so users can seamlessly transition back to their analysis workflow. Also let the mouse be used normally again. 
Fixed: Added `flyModeActive` state + `flyModeActiveRef` + `toggleFlyMode` callback in Visualizer.jsx. `toggleFlyMode` saves/restores events panel, settings panel, and detail panel open/closed state. F key added to `useKeyboardShortcuts.js` (only F works when `flyModeActiveRef.current` is true — all other shortcuts suppressed). WASD keys now only respond when `flyModeActiveRef.current` is true (gated at top of `onKeyDown` in `useWASDNavigation.js`). Pan-down key changed from 'f' to 'c' to free up F. A `fly-mode-banner` div is rendered when fly mode is active (shows W/A/S/D/R/C/Q/E hint). CSS added in 17-misc.css: banner styling + crosshair cursor in fly mode.

244 In the animation settings, make the option to animate "Changed bits" vs "Targeted bits". When "Changed bits" is selected, the animation should show the bits that actually changed in the marked numbers as a result of the event. When "Targeted bits" is selected, the animation should show the bits that were targeted for change by the event, even if they were already set and did not change. This way users can choose to focus on either the actual changes that occurred or the intended targets of the events, depending on their analysis needs and preferences. The animation for targeted bits must make clear if a bit was already set or not, for example by using different colors or animation styles for bits that were newly set vs bits that were already set. This way users can easily distinguish between the two types of bits and understand the cumulative effect of the events on the marked numbers.
Fixed: Added `animateBitsMode` state ('changed'|'targeted') + `animateBitsModeRef` in `useAnimationConfig.js`. Exposed in `animationConfigContextValue`. Passed `animateBitsModeRef` to `useGoToStep`. In `useGoToStep.js`: when mode is 'targeted', `changedSetForRender = targetSet` and `repeatedBitsForRender = targetSet \ changedSet` (bits in target that were already set → shown in amber via the existing `repeatedBits` renderer path). Animation trigger also uses `targetSet` in targeted mode. Added "Animate bits" section in `AnimationTab.jsx` with two `PreviewOptionButton` choices. Connected in `SettingsPanel.jsx`.

245 Make shortcut keys: e for toggle events panel, s for toggle settings panel and d for toggle detail panel. L for Log. 
Fixed: Added E (toggle events panel), S (toggle settings panel), L (open raw log) to `useKeyboardShortcuts.js`. D for detail already existed. Updated `KeyboardShortcutsOverlay.jsx` SHORTCUTS array. Added `toggleEventsPanel`, `toggleSettingsPanel`, `openRawLog` callbacks wired from `Visualizer.jsx`. Also added F (fly mode toggle). In fly mode, all shortcuts except F are suppressed.

246 Make a zoom-in function for the animation timeline, where the users can select a slice of the timeline which they can then scrub with more precision. This way users can focus on a specific portion of the animation timeline and analyze it in more detail, especially when dealing with long animations with many events. The zoomed-in slice should have its own scrubber that allows users to navigate through that portion of the timeline with finer granularity, while still showing the overall context in the main timeline.
Fixed: Added `animZoomRange` state (null | [start, end] fractions) in `DoubleTimeline.jsx`. Ctrl+drag (or Cmd+drag on Mac) on the animation zone selects a zoom range, shown as a highlighted overlay. When zoom is active, the main scrubber maps clicks/drags into the zoomed slice for fine-grained seeking. A secondary zoom scrubber row appears below the track showing position within the zoomed range, with a reset (×) button. Double-click the main anim zone to clear zoom. CSS added in 24-double-timeline.css for zoom overlay, zoom scrubber track/fill/head, and zoomed zone border highlight.
