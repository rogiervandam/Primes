...
Encapsulates the `keydown` global listener for single-key shortcuts (play/pause, step, seek, zoom, theme toggle, etc.). `handlers` is an object of named callbacks supplied by `Visualizer`. All inputs are stored in an internal `handlersRef` that is updated on every render; the listener reads from the ref and is registered **once** (empty dep array), so it never tears down and re-registers during playback or step navigation.
