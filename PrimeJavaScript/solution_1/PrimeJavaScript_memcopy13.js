...
//		let range = patternsize_bits * 2;
		let range = 1;
		let copyrange_stop = block_start;
		...
		do  {
			...
			if (range < block_range) { // check if we should copy previous results
				...
				if (range > block_range) { // at the maximum of the pattern that can fill the block
					...
					// copyrange_stop = block_start + block_range;
					if (block_start==0) {
						copyrange_stop = block_start + block_size + block_range; // copy to the next block
					}
					else {
						copyrange_stop = this.bitArray.size;
					}
					if (copyrange_stop > this.bitArray.size) copyrange_stop = this.bitArray.size;
					// copyrange_stop = block_start + block_size + block_size - 1;
					// console.log(`Copying mid patternsize ${block_start}-${block_stop} (pattern:${patternsize_bits} range: ${range} block_range ${block_range}) to next block ${block_stop+1}-${copyrange_stop} with prime ${block_start_prime} embedded`);
				}
				else {
					copyrange_stop = block_start + range;
					block_start_prime = prime; // memorize the last prime that was embedded in de range
				}

				// console.log(`copyrange set to ${copyrange_stop} start ${block_start} range ${range} block_range ${block_range}`);

				if (patternsize_bits>1) {
					if (block_start==0) {
						this.bitArray.copyPattern(block_start+patternsize_bits, patternsize_bits, copyrange_stop);
						// console.log(`executing copy... ${block_start} ${patternsize_bits} ${copyrange_stop}`);
					}
					else {
			...
			if (prime > prime_start) {
				// console.log(`Striping ${block_start}-${block_stop} with ${prime}`);
				this.bitArray.setBitRangeTrue(start, step, block_start+range);
			}
		} while (range < block_range);
	...
	runBlock(block_start, block_stop, prime) {
		while (true) {
			prime = this.bitArray.searchBitFalse(prime + 1);
			let step = prime * 2 + 1;
			let start = prime + prime + (prime * prime * 2);
	...
	runSieve(blocksize) {
		...
		do {
			...
			this.runBlock(block_start, block_stop, block_start_prime);
			block_start += blocksize;
			block_stop += blocksize;
		...
		// 		block_stop = this.sieveSizeInBits;
		// 	}
		// 	// this.runBlock(block_start, block_stop, 0);
		// 	block_start_prime = this.runBlock_memcopy(block_start, blocksize, block_start_prime);
		// 	block_start += blocksize;
		// 	block_stop += blocksize;
		// } while (block_start < this.sieveSizeInBits)
