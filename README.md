...
## Command-line Options for `sieve`

The `sieve` script supports various command-line options to customize its behavior. Below are some of the key options:

- `--chain`: Enables chain mode, which modifies the sieve's execution flow for specific use cases.
- `--embed`: Activates embedded mode.
- `--timing`: Enables timing mode to measure execution time.
- `--explain`: Provides an explanation of the sieve's operations.
- `--visualize`: Activates visualization mode for better understanding of the sieve process.
- `--trace`: Enables trace logging for debugging purposes. Requires a trace level (5-9) to follow it.

For a complete list of options and their usage, refer to the script or run it with the `--help` flag.
